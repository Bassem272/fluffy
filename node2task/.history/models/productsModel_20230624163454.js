...
return dataUser;
 ...
 async saveCategories(products) {
  const groupedProducts = await groupBy(products, 'category'); let id = 1; const idCategoryArray = await Object.keys(groupedProducts).map((category) => ({ id: id++, name: category })); await fs.writeFile('categories.json',JSON.stringify( idCategoryArray)); return idCategoryArray;},
