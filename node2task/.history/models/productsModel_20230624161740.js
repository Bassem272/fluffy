...
return dataUser;
 ...
 async saveCategories(products) {
  const groupedProducts = await groupBy(products, 'category');
  let id = 1;
const idCategoryArray = await Object.keys(groupedProducts).map((category) => ({
  id: id++,
  name: category
}));
    //const CategoryArray = await  products.map(({ category,id }) => ({ name : category,id}));
    await fs.writeFile('categories.json',JSON.stringify( idCategoryArray));
